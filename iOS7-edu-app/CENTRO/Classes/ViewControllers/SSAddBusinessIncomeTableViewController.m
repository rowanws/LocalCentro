//
//  SSAddBusinessIncomeTableViewController.m
//  CENTRO
//
...
@interface SSAddBusinessIncomeTableViewController () <NSFetchedResultsControllerDelegate> {
...
@implementation SSAddBusinessIncomeTableViewController
...
-(void) prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender {
    if([segue.identifier isEqualToString:@"AddBusinessIncomeDetail"]) {
        ...
        Product *pro = (Product *) [_fetchedResultsController objectAtIndexPath:self.selectedIndexPath];
        
        [addEditBusinessIncomeViewController setItem:pro];
        [addEditBusinessIncomeViewController setEditingExistingIncome:NO];
    }
}
...
-(void) viewDidAppear:(BOOL)animated {
    ...
    [fetchRequest setPredicate:[NSPredicate predicateWithFormat:@"selectedAsProfitableItem = 0 AND name != %@", @""]];
...
{
    static NSString *CellIdentifier = @"AddBusinessIncomeCell";
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    [self performSegueWithIdentifier:@"AddBusinessIncomeDetail" sender:self];
}
