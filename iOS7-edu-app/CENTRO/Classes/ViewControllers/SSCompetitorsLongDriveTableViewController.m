//
//  SSCompetitorsLongDriveTableViewController.m
//  CENTRO
//
...
#import "SSCompetitorsLongDriveTableViewController.h"
#import "CompetitorProfile.h"
#import "SSUser.h"
#import "SSUtils.h"

@interface SSCompetitorsLongDriveTableViewController () <NSFetchedResultsControllerDelegate> {
...
@implementation SSCompetitorsLongDriveTableViewController
...
- (IBAction)nextButtonPressed:(id)sender {
    ...
    [self performSegueWithIdentifier:@"CompetitorsOnline" sender:self];
}
...
- (void)viewDidLoad
{
    ...
    self.titleLabel.text = [NSString stringWithFormat:@"Which competitors sell to people within a long drive of %@? (Select all that apply)", [SSUtils companyName]];
}
...
{
    static NSString *CellIdentifier = @"CompetitorLongDriveCell";
    ...
    if([temp.sellLongDrive isEqualToNumber:[NSNumber numberWithBool:NO]]) {
        [cell setAccessoryType:UITableViewCellAccessoryNone];
    } else {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if([temp.sellLongDrive isEqualToNumber:[NSNumber numberWithBool:NO]]) {
        [temp setSellLongDrive:[NSNumber numberWithBool:YES]];
    } else {
        [temp setSellLongDrive:[NSNumber numberWithBool:NO]];
    }
