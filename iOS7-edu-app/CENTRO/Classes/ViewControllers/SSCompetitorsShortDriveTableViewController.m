//
//  SSCompetitorsShortDriveTableViewController.h
//  CENTRO
//
...
#import "SSCompetitorsShortDriveTableViewController.h"
#import "CompetitorProfile.h"
#import "SSUser.h"
#import "SSUtils.h"

@interface SSCompetitorsShortDriveTableViewController () <NSFetchedResultsControllerDelegate> {
...
@implementation SSCompetitorsShortDriveTableViewController
...
- (IBAction)nextButtonPressed:(id)sender {
    ...
    [self performSegueWithIdentifier:@"CompetitorsLongDrive" sender:self];
}
...
//UIImageView *backgroundImageView40 = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"bg-568h@2x.png"]];
  //  UIImageView *backgroundImageView35R = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"bg@2x.png"]];
    ...
    self.titleLabel.text = [NSString stringWithFormat:@"Which competitors sell to people within a short drive of %@? (Select all that apply)", [SSUtils companyName]];
}
...
{
    static NSString *CellIdentifier = @"CompetitorShortDriveCell";
    ...
    if([temp.sellShortDrive isEqualToNumber:[NSNumber numberWithBool:NO]]) {
        [cell setAccessoryType:UITableViewCellAccessoryNone];
    } else {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if([temp.sellShortDrive isEqualToNumber:[NSNumber numberWithBool:NO]]) {
        [temp setSellShortDrive:[NSNumber numberWithBool:YES]];
    } else {
        [temp setSellShortDrive:[NSNumber numberWithBool:NO]];
    }
