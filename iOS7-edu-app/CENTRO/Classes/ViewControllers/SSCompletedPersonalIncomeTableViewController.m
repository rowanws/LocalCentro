//
//  SSCompletedPersonalIncomeTableViewController.m
//  CENTRO
//
...
#import "SSCompletedPersonalIncomeTableViewController.h"
#import "SSUser.h"
#import "StudentIncome.h"

@interface SSCompletedPersonalIncomeTableViewController () <NSFetchedResultsControllerDelegate> {
...
@implementation SSCompletedPersonalIncomeTableViewController
...
- (IBAction)tryAgainButtonPressed:(id)sender {
    [[SSUser currentUser] setActivityNumberAsIncompleted:@"7"];
    
    UIStoryboard *profileStoryBoard = [UIStoryboard storyboardWithName: @"personal_income" bundle:Nil];
...
- (void)viewDidLoad
{
    ...
    self.navigationItem.title = @"Per. Income";
    ...
    [fetchRequest setPredicate:[NSPredicate predicateWithFormat:@"selectedAsSourceOfIncome = 1"]];
...
{
    static NSString *CellIdentifier = @"CompletedPersonalIncome";
    ...
    double doubleAmount = [self monthlyAmountWithFrequency:[managedObject valueForKey:@"frequency"] forIncomeCode:[managedObject valueForKey:@"studentIncomeCode"] andAmount:[managedObject valueForKey:@"amount"]];
...
-(double) monthlyAmountWithFrequency:(NSNumber *) frequency forIncomeCode:(NSString *) incomeCode andAmount:(NSNumber *) amount {
    double monthlyAmount = 0.0;
    
    if([frequency isEqual:[NSNumber numberWithInt:1]]) {
        if([incomeCode isEqualToString:@"SIN_JOB"] || [incomeCode isEqualToString:@"SIN_2JOB"]) {
            monthlyAmount = [amount doubleValue] * 21;
        } else {
