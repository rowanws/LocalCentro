//
//  SSEditCompetitorsPrioritiesTableViewController.m
//  CENTRO
//
...
@interface SSEditCompetitorsPrioritiesTableViewController () <NSFetchedResultsControllerDelegate> {
...
@implementation SSEditCompetitorsPrioritiesTableViewController
...
{
    static NSString *CellIdentifier = @"EditCompetitorRankCell";
...
{
    self.competitors = [[_fetchedResultsController fetchedObjects] mutableCopy];

    NSManagedObject *competitor = [_fetchedResultsController objectAtIndexPath:sourceIndexPath];
    
    [self.competitors removeObject:competitor];
    
    [self.competitors insertObject:competitor atIndex:[destinationIndexPath row]];
    
    int i = 1;
    for (NSManagedObject *mo in self.competitors)
    {
        if(self.priority == 1) {
