//
//  SSPersonalIncomeTableViewController.m
//  CENTRO
//
...
@interface SSPersonalIncomeTableViewController () <NSFetchedResultsControllerDelegate> {
...
@implementation SSPersonalIncomeTableViewController
    ...
    if (viewControllers.count > 1 && [viewControllers objectAtIndex:viewControllers.count-2] == self) {
        ...
        @try {
            if(self.popToRoot && !self.doneButtonPressed) {
                [[SSUser currentUser] setSegueToPerform:@"7"];
...
{
    if ([[[_fetchedResultsController sections] objectAtIndex:section] numberOfObjects] == [self totalIncomeCategories]) {
...
{
    static NSString *CellIdentifier = @"PersonalIncomes";
...
{
    if(![indexPath isEqual:self.addItemIndexPath]) {
        [self performSegueWithIdentifier:@"EditPersonalIncome" sender:(StudentIncome *) [_fetchedResultsController objectAtIndexPath:indexPath]];
    } else {
        [self performSegueWithIdentifier:@"AddPersonalIncome" sender:self];
    }
...
-(void) prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender {
    ...
    if([segue.identifier isEqualToString:@"EditPersonalIncome"]) {
        ...
        [addEditPersonalIncomeViewController setStudentIncome:(StudentIncome *) sender];
        [addEditPersonalIncomeViewController setEditingExistingIncome:YES];
    }
}
